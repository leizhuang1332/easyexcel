...
import com.alibaba.excel.analysis.ScriptAnalyserImpl;
...
public class ExcelHeadProperty {
    ...
    public ExcelHeadProperty(Holder holder, Class headClazz, List<List<String>> head, Boolean convertAllFiled, Map<String, Object> param, Translator translator) {
        ...
        initColumnProperties(holder, convertAllFiled, param, translator);
    ...
    private void initColumnProperties(Holder holder, Boolean convertAllFiled, Map<String, Object> param, Translator translator) {
        if (headClazz == null) {
            return;
        }
        // Declared fields
        ...
        for (Map.Entry<Integer, Field> entry : sortedAllFiledMap.entrySet()) {
            initOneColumnProperty(entry.getKey(), entry.getValue(), indexFiledMap.containsKey(entry.getKey()), param, translator);
        }
     ...
     */
    private void initOneColumnProperty(int index, Field field, Boolean forceIndex, Map<String, Object> param, Translator translator) {
        ...
        if (headMap.containsKey(index)) {
            ...
            if (notForceName) {
                ...
                if (param != null ) {
                    ExcelPropertyConditionMulti[] multi = excelProperty.multi();
                    for (ExcelPropertyConditionMulti excelPropertyConditionMulti : multi) {
                        String test = excelPropertyConditionMulti.test();
                        boolean eval = new ScriptAnalyserImpl(test, param).parse().eval();
                        if (eval) {
                            String[] value = excelPropertyConditionMulti.value();
                            if (translator != null) {
                                value = translator.apply(excelPropertyConditionMulti.value());
                            }
                            Collections.addAll(tmpHeadList, value);
                            isMulti = true;
                            break;
                        }
                    }
                ...
                if (!isMulti) {
                    ...
                    if (translator != null) {
                        value = translator.apply(excelProperty.value());
                    }
                    Collections.addAll(tmpHeadList, value);
                }
            }

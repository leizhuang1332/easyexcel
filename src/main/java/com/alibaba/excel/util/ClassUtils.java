...
public class ClassUtils {
     ...
     * 缓存字段，默认开启
     */
    public static boolean IS_FIELD_CACHE= true;
    ...
    public static void declaredFields(Class clazz, Map<Integer, Field> sortedAllFiledMap,
        ...
        Boolean needIgnore, Holder holder, Map<String, Object> param) {
        FieldCache fieldCache = getFieldCache(clazz, convertAllFiled, param);
        if (fieldCache == null) {
            return;
        }
        if (ignoreMap != null) {
    ...
    public static void declaredFields(Class clazz, Map<Integer, Field> sortedAllFiledMap, Boolean convertAllFiled,
        ...
        declaredFields(clazz, sortedAllFiledMap, null, null, convertAllFiled, needIgnore, writeHolder, null);
    }

    private static FieldCache getFieldCache(Class clazz, Boolean convertAllFiled, Map<String, Object> param) {
        if (clazz == null) {
            return null;
        }
        SoftReference<FieldCache> fieldCacheSoftReference = FIELD_CACHE.get(clazz);
        if (IS_FIELD_CACHE && fieldCacheSoftReference != null && fieldCacheSoftReference.get() != null) {
        ...
        synchronized (clazz) {
            ...
            declaredFields(clazz, convertAllFiled, param);
        }
        return FIELD_CACHE.get(clazz).get();
    }

    private static void declaredFields(Class clazz, Boolean convertAllFiled, Map<String, Object> param) {
        ...
        for (Field field : tempFieldList) {
            declaredOneField(field, orderFiledMap, indexFiledMap, ignoreMap, excelIgnoreUnannotated, convertAllFiled, param);
        }
    ...
    private static void declaredOneField(Field field, Map<Integer, List<Field>> orderFiledMap,
        ...
        Boolean convertAllFiled, Map<String, Object> param) {
        ExcelIgnore excelIgnore = field.getAnnotation(ExcelIgnore.class);
        if (excelIgnore != null) {
            ignoreMap.put(field.getName(), field);
            return;
        } else {
            ExcelIgnoreCondition excelIgnoreCondition = field.getAnnotation(ExcelIgnoreCondition.class);
            if(excelIgnoreCondition != null && param != null){
                String test = excelIgnoreCondition.test();
